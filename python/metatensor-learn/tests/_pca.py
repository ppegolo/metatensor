...
from metatensor.learn.decomposition import EquivariantPCA  # noqa: E402
...
def test_equivariance(tensor, wigner_d_real):
    """
    Tests that application of the PCA is equivariant to O3 transformation of the input.
    """
    # Define input and rotated input
    x = tensor
    Rx = wigner_d_real.transform_tensormap_o3(x)

    # Define the PCA calculator
    epca = EquivariantPCA(
        n_components=10,
        key_lambda_name="o3_lambda",
    )

    #
    Rfx = wigner_d_real.transform_tensormap_o3(epca.fit_transform(x))  # R . epca(x)
    fRx = epca.fit_transform(Rx)  # epca(R . x)
