...
def sort_by(
    ...
    :param axes: axes to sort. The labels entries along these axes will be sorted in
        lexicographic order, and the arrays values will be reordered accordingly.
        Possible values are ``'keys'``, ``'samples'``, ``'components'`` and
        ``'properties'``.
    :param name: name of `axes` to be used for the sorting.
    :param descending: if false, the order is ascending. Default False
    ...
    ... )
    >>> metatensor.sort(tensor, axes="keys",names="types")
    TensorMap with 2 blocks
    keys: types
            0
            1
    """
    if axes not in ["keys","samples", "components", "properties"]:
        raise ValueError(
                    "'axes' must be a string between "
                    "'keys', 'samples', 'components', 'properties'"
                )
        
    if axes=="keys":
        # Do we need to sort the keys?
        axis = _get_index(name,tensor.keys.names)
        if axis==-1:
            raise ValueError(
                        "'name' must be a valid name for keys "+
                        f"but 'name'={name} and "+
                        f"tensor.keys.names = {tensor.keys.names}"
                    )
            
        sorted_idx = _dispatch.argsort(
            tensor.keys.values,dim=axis reverse=descending
        )
        new_keys = Labels(
        ...
        for i in sorted_idx:
            new_blocks.append(
                    tensor.block(tensor.keys[int(i)]),
            )
    else:
        new_keys = Labels(
            names=tensor.keys.names,
            values=tensor.keys.values,
        )
